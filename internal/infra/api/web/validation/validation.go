...
func ValidateErr(validation_err error) *rest_err.RestErr {
	...
	if errors.As(validation_err, &jsonErr) {
		return rest_err.NewNotFoundError("Invalid JSON")
		...
		return rest_err.NewBadRequestError("Invalid input fields", errorCauses...)
	} else {
		return rest_err.NewInternalServerError("Conversion error")
	}
}
